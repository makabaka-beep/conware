...
def create_minimal_startup_file(proj_root):
    """创建极简STM32F407启动文件（解决Reset_Handler链接失败）"""
    startup_file = os.path.join(proj_root, "startup_stm32f407.s")
    if os.path.exists(startup_file):
        log_info(f"Startup file already exists: {startup_file}")
        return startup_file
    
    # 极简启动文件内容（仅包含链接必需的Reset_Handler和栈初始化）
    startup_content = """
.syntax unified
.cpu cortex-m4
.fpu fpv4-sp-d16
...
"""
    ...
    log_info(f"Created minimal startup file: {startup_file}")
    return startup_file


def run_command(cmd, output_file):
    """增强版：执行命令并捕获输出（兼容中文路径+GBK编码注释）"""
    try:
        # 拆分多行命令，确保执行顺序
...
def run_stm32_build(build_dir, repo_dir, main_file_path, build_output_file):
    """最终版STM32裸机编译（修复参数+补全头文件+编码兼容+链接启动文件）"""
    # 获取STM32工程根目录（main.c所在目录，适配中文路径）
    proj_root = os.path.dirname(main_file_path)
    
    # 第一步：创建极简启动文件（解决Reset_Handler链接失败）
    startup_file = create_minimal_startup_file(proj_root)
    ...
    cflags = (
        ...
        "-fno-builtin -fno-exceptions"                           # 移除-fno-rtti
    )
    ...
    # 编译命令（适配中文路径，显式写入日志文件）
    compile_cmd = f"arm-none-eabi-gcc {cflags} {includes} {defines} -c {main_file_path} -o {build_dir}/main.o"
    # 编译启动文件（汇编文件）
    compile_startup_cmd = f"arm-none-eabi-as -mcpu=cortex-m4 -mthumb -mfpu=fpv4-sp-d16 -mfloat-abi=softfp {startup_file} -o {build_dir}/startup.o"
    # 链接命令：添加编译后的启动文件，移除--entry（启动文件已指定入口）
    link_cmd = f"arm-none-eabi-gcc {cflags} -Wl,-Ttext=0x08000000 {build_dir}/startup.o {build_dir}/main.o -lgcc -o {build_dir}/stm32_project.elf"
...
def perform_llvm_transformation(llvm_bc_out, original_build_dir, compile_json_out, clang_path, opt_path, so_path):
    ...
    try:
        ...
        # 安全解码输出
        log_info(f"LLVM transformation output: {safe_decode(result.stdout.encode())}")
        log_success("Finished running LLVM transformations for STM32.")
        return True
    except subprocess.CalledProcessError as e:
        log_error(f"LLVM transformation failed: {safe_decode(e.stderr.encode())}")
        log_error(f"LLVM command output: {safe_decode(e.stdout.encode())}")
...
def main():
    ...
    # 检查arm-none-eabi工具链（新增检查汇编器as）
    arm_gcc_path = get_bin_path("arm-none-eabi-gcc")
    arm_as_path = get_bin_path("arm-none-eabi-as")
    if not arm_gcc_path:
        log_error("arm-none-eabi-gcc not found! Please install STM32 toolchain.")
        sys.exit(-1)
    if not arm_as_path:
        log_error("arm-none-eabi-as not found! Please install STM32 toolchain.")
        sys.exit(-1)
